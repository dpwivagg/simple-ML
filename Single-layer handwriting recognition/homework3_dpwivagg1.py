...
def compute_L(x, y, W, alpha):
    ...
    w_dot_w = np.array([np.dot(W[i], W[i].T) for i in range(W.shape[0])])
...
def train(X, Y, batch_size, epsilon, n_epochs, alpha):
    ...
    for epoch in range(n_epochs):
        ...
        for i in pbar:
            ...
            dL_dW = compute_gradient(x, y, W)
            W = update_W(W, dL_dW, epsilon)
...
W, L = train(X, Y, batch_size=75, epsilon=0.07, n_epochs=2, alpha=.005)
