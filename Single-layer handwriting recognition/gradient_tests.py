...
def forward(x, y, W, alpha):
    ...
    a = y_hat / y_hat.sum()
    n = len(y)
    reg = (alpha / 2) * (W.T * W).sum()
    cross_loss = np.array([y[i] * np.log(y[i]).item() for i in range(n)])
